...
A fourth stage, **DeployProd**, is the same shape against a genuine `prod` bundle target — `databricks.yml` adds `targets.prod` with `mode: production`, an explicit `workspace.root_path` (required by production mode; the CLI's own validation error names the exact value to use), and the `schema` variable overridden to `fraud_detection_prod`. There's no second Databricks workspace here, so `prod` is a distinct schema (with its own `raw_landing`/`checkpoints` volumes) in the same Free Edition workspace as `dev`, not a separate deployment target in the infrastructure sense — but it exercises the real multi-environment bundle/pipeline mechanics: `${bundle.target}`-suffixed resource names keep `fraud_detection_job_dev` and `fraud_detection_job_prod` from colliding, and `databricks-prod` is a second Environment with its own independent approval gate, so approving a dev deploy never implicitly approves prod.

Boards tracks the project's open bugs and backlog as work items (mirroring `docs/DESIGN.md` §6/§7.3) rather than only living in this document.
